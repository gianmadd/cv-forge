...
- **Zero fabrication.** Every fact, metric, responsibility, and claim comes from the user. Nothing is invented, estimated, or embellished. If the user doesn't have a number, it's left out — never suggested. **Durations and counts follow the same rule:** any tenure, "X years as Y", or team size must be stated by the user or directly datable from Work Experience — never a total decomposed into per-role figures, and (in `cv-tailor`) never distinct roles aggregated to match a job posting. The rule constrains the *tool*, not the user: it forbids the tool from inventing or inflating on its own initiative — it is **not** a licence to doubt, challenge, or fact-check the user's own self-assessment. The user is the source of truth for their own facts; when they ask to record something they themselves flagged as untrue, the tool offers honest framing once and then respects their decision — it does not lecture or overrule.
